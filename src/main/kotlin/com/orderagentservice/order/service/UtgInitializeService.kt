...
class UtgInitializeService @Autowired constructor(
    ...
    fun initializeGraph(
        ...
        var isNext = true
        val actionList = mutableListOf<AgentActionDto>()
        val rootUiDto = UiDto(
            isNext = true,
            x = -1,
            y = -1,
        ...
        while (true) {
            ...
            if (isNext == true) {
                llmUiList.clear()
                llmUiList = getUiComponents(kioskId)
                isNext = false
                continue
            }
            println(llmUiList)
            ...
            val action = actionAgent.determineAction(menuDto, llmUiList)
            ...
            val llmOptList = getUiComponents(kioskId)
            val backAction = backAgent.determineBack(llmOptList)
            for (opt in menuDto.options) {
                val optAction = actionAgent.determineAction(MenuInfoDto(opt, listOf(), menuDto.title), llmOptList)
    ...
    private fun getUiComponents(kioskId: String): MutableList<LlmUiComponentDto> {
